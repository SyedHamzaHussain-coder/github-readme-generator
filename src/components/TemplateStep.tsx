...
export const TemplateStep: React.FC<TemplateStepProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto px-2 sm:px-4">
      <div className="text-center mb-8 sm:mb-12">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent mb-2 sm:mb-4">
          Choose Your {readmeType === 'repository' ? 'Repository' : 'Profile'} Template
        </h2>
        <p className="text-gray-600 text-base sm:text-lg">Select a template that matches your style</p>
      </div>
  ...
  <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 mb-6 sm:mb-8 border border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2 sm:gap-0">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800">Select Repository</h3>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h4 className="font-semibold text-gray-800 text-base sm:text-lg">{repo.name}</h4>
                      <p className="text-gray-600 text-xs sm:text-sm">{repo.description}</p>
                      <div className="flex flex-wrap items-center space-x-2 sm:space-x-4 mt-1 sm:mt-2 text-xs sm:text-sm text-gray-500">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        {templates.map((template) => (
          <div
            ...
            className={`bg-white rounded-2xl shadow-xl p-4 sm:p-6 cursor-pointer transition-all duration-300 border-2 ${
          ...
          >
            <div className="text-center mb-2 sm:mb-4">
              <Wand2 className={`w-7 h-7 sm:w-8 sm:h-8 mx-auto mb-2 sm:mb-3 ${
                selectedTemplate === template.id ? 'text-primary' : 'text-gray-400'
              }`} />
              <h3 className="font-bold text-gray-800 text-base sm:text-lg">{template.name}</h3>
              <p className="text-xs sm:text-sm text-gray-600">{template.description}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-2 sm:p-3 text-xs font-mono text-gray-700 overflow-hidden max-h-24 sm:max-h-32">
      ...
      <div className="text-center space-y-3 sm:space-y-4">
        <div className="flex justify-center">
          <button
            ...
            className="bg-gradient-to-r from-primary to-secondary text-white px-6 sm:px-8 py-3 sm:py-4 rounded-full font-semibold hover:shadow-2xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
          >
